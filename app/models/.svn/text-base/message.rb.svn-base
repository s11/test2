...
# == Message visibility
# 
# Although you can delete a message, it will also delete it from the inbox of all the message's recipients. Instead, you can change
# the *visibility* of the message. Messages have 1 of 2 states that define its visibility:
...
class Message < ActiveRecord::Base
  ...
  # Handle simple message chains ie. reply_to etc
  ...
  # Validations --------------------------------------------------------------------------------------------------------------------
  validates_presence_of :state, :sender_id, :sender_type, :subject, :body
  ...
  state_machine :state, :initial => :unsent do
    before_transition [:unsent, :queue] => :sent, :on => :deliver, :do => :expand_class_group_recipients
    ...
    # Sends the message to all of the recipients as long as at least one recipient has been added
    event :deliver do
  ...
  def forward
    message        = self.class.new(:subject => "Fwd: #{subject}", :body => body)
    message.sender = sender
    message
  end
  
  # Replies to this message, including the original subject and body in the new message.  Only the original direct receivers are
  # added to the reply.
  def reply
    message                = self.class.new(:subject => "Re: #{subject}", :body => "#{sender.fullname} wrote:\n\n#{body.andand.gsub(/^/, '> ')}\n\n")
  ...
  # Replies to all recipients on this message, including the original subject and body in the new message.  All receivers (direct, 
  # cc, and bcc) are added to the reply.
  def reply_to_all
    message = reply
    message.cc(cc)
    message.bcc(bcc)
  ...
  private
    ...
    # Expands / replaces recipient class groups to normal users. Called when the email transitions to :sent.
    def expand_class_group_recipients
      if recipients.class_groups.count > 0
        users = recipients.class_groups.map(&:receiver).map(&:users).flatten.uniq.compact
        to(users)
      end
    end
