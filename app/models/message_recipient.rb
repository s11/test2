...
class MessageRecipient < ActiveRecord::Base
  ...
  named_scope :class_groups,  :conditions => { :receiver_type => 'ClassGroup' }
  named_scope :users,         :conditions => { :receiver_type => 'User'       }
  ...
  # message
  def forward
    message        = self.message.class.new(:subject => "Fwd: #{subject}", :body => body)
  ...
  def reply
    message                = self.message.class.new(:subject => "Re: #{subject}", :body => "#{sender.fullname} wrote:\n\n#{body.andand.gsub(/^/, '> ')}\n\n")
    message.parent_message = self.message
    message.sender         = receiver
    message.to(sender)
    message
  end
